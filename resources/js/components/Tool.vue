...
<div align='right'>
	...
	<button @click="fire" class="btn btn-default btn-icon bg-primary text-white">
		<span v-if="this.panel.fields[0].button_text ? true : false">{{ this.panel.fields[0].button_text }}</span>
		<svg v-if="this.panel.fields[0].button_text ? false : true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="22" height="22" class="fill-current">
			<path
			...
			<div class="border-t border-50 px-6 py-3 ml-auto flex items-center" style="min-height: 70px; flex-direction: row-reverse;">
				...
				<span>
					<span @click="proceed" class="nova-button nova-button-post cursor-pointer dim inline-block text-primary font-bold no-underline nova-button-post cursor-pointer dim inline-block text-primary font-bold no-underline"
						...
						{{ this.panel.fields[0].confirm_button_text ? this.panel.fields[0].confirm_button_text : 'Clone'}}
					</span>
				</span>
			</div>
		</div>
...
export default {
	...
	methods: {
		...
		proceed() {
			...
			axios
				.post("/nova-vendor/clone-model/endpoint", {
